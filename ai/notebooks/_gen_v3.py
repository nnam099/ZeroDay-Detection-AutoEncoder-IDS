...
# ── PART 4: Preprocessing + Split + Scaler ───────────────────────────────────
PARTS.append('''
# ─────────────────────────────────────────────────────────────────────────
# 4. TIỀN XỬ LÝ
# ─────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────
# 5. SPLIT + BOUNDARY BUFFER [FIX-4]
# ─────────────────────────────────────────────────────────────────────────
print("\\n[SPLIT] Tách trước khi tạo windows + buffer biên...")
...
BUF = SEQ - 1
def trim_head(idx, ref, buf=BUF):
    if len(ref)==0: return idx
    return idx[idx > ref[-1] + buf]

idx_vl = trim_head(idx_vl, idx_tr)
idx_ts = trim_head(idx_ts, np.concatenate([idx_tr, idx_vl]))
...
# ── PART 8: Threshold Tuning + Evaluation ────────────────────────────────────
PARTS.append('''
# ─────────────────────────────────────────────────────────────────────────
# 12. THRESHOLD TUNING
# ─────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────
# 14. VISUALIZATIONS [FIX-5: ROC dùng ts data]
# ─────────────────────────────────────────────────────────────────────────
...
axes[0,2].axvline(THR_CLS, color="red", ls="--", lw=2, label=f"thr={THR_CLS:.3f}")
axes[0,2].set_title("P(Attack) Distribution"); axes[0,2].legend()

# RE dist
re_b = ts_re[ts_true==0]; re_a = ts_re[ts_true==1]
axes[1,0].hist(re_b, bins=100, alpha=.6, label="BENIGN", color="steelblue", density=True)
axes[1,0].hist(re_a, bins=100, alpha=.6, label="Attack", color="tomato",    density=True)
axes[1,0].axvline(THR_RE, color="red", ls="--", lw=2, label=f"thr_re={THR_RE:.4f}")
axes[1,0].set_title("[FIX-2] RE Distribution (benign-only training)")
axes[1,0].legend(); axes[1,0].set_xlabel("MSE(x, x̂)")

# 2D scatter
si = np.random.choice(len(ts_true), min(5000,len(ts_true)), replace=False)
colors = np.where(ts_true[si]==0,"steelblue","tomato")
axes[1,1].scatter(ts_probs[si], ts_re[si], c=colors, alpha=.3, s=8)
axes[1,1].axvline(THR_CLS, color="blue", ls="--", lw=1.5, label=f"thr_cls={THR_CLS:.3f}")
axes[1,1].axhline(THR_RE,  color="red",  ls="--", lw=1.5, label=f"thr_re={THR_RE:.4f}")
axes[1,1].set_xlabel("P(Attack)"); axes[1,1].set_ylabel("Reconstruction Error")
axes[1,1].set_title("2D Decision Space"); axes[1,1].legend()
axes[1,1].text(0.05, THR_RE*1.5, "ANOMALY",  fontsize=9, color="darkred")
axes[1,1].text(THR_CLS+.02, 0, "ATTACK",    fontsize=9, color="navy")
axes[1,1].text(0.05, 0, "BENIGN",            fontsize=9, color="steelblue")

# ROC — [FIX-5] dùng fpr_ts/tpr_ts (test set thực sự)
axes[1,2].plot(fpr_ts, tpr_ts, color="darkorange", lw=2, label=f"Test AUC={ts_auc:.4f}")
axes[1,2].plot([0,1],[0,1], "k--", alpha=.3)
axes[1,2].set_title("ROC Curve (Test Set)"); axes[1,2].legend()
axes[1,2].set_xlabel("FPR"); axes[1,2].set_ylabel("TPR"); axes[1,2].grid(alpha=.3)

plt.suptitle("DDoS V3-Improved: Known Attack + Zero-Day", fontsize=13, y=1.01)
plt.tight_layout()
plt.savefig(f"{RESULT_DIR}/v3i_evaluation.png", dpi=150, bbox_inches="tight")
print(f"Saved evaluation → {RESULT_DIR}/v3i_evaluation.png")
plt.close()
''')

# ── PART 10: Summary + Inference ─────────────────────────────────────────────
PARTS.append('''
# ─────────────────────────────────────────────────────────────────────────
# 15. TÓM TẮT
# ─────────────────────────────────────────────────────────────────────────
