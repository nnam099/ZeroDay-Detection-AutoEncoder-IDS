...
class CFG:
    ...
    batch_size  = 512
    lr          = 3e-4
    hidden      = 256       # tăng capacity
    ae_hidden   = 128       # AutoEncoder hidden
    patience    = 20

    # Loss
    lambda_con  = 0.3       # giảm để focal loss chiếm vai trò chính
    focal_gamma = 2.0       # giảm để tránh loss quá lớn
    dos_weight  = 8.0       # khôi phục 8.0: AE tđ mạnh hơn để bắt Zero-Day

    # Zero-day
    target_fpr        = 0.05   # nới rộng → bắt được nhiều ZD hơn
    n_clusters        = 25
    zd_augment_factor = 1
...
def engineer_features(df, feat_cols):
    ...
    # TCP handshake quality (DoS SYN flood → synack/ackdat 0)
...
def prepare_splits(df, known_cats=KNOWN_ATTACK_CATS, zd_cats=ZERO_DAY_ATTACK_CATS,
    ...
    clip = 10.   # giảm clip range để tăng stability
    X_tr, X_va, X_te, X_zd = [np.clip(x, -clip, clip) for x in [X_tr, X_va, X_te, X_zd]]
    # Thay NaN/Inf còn sót (do feature engineering) bằng 0
    X_tr, X_va, X_te, X_zd = [
...
class AutoEncoder(nn.Module):
    """AE trained on Normal + Known attacks → high RE on Zero-day."""
...
class IDSModel(nn.Module):
    ...
    def attack_prob(self, x):
        ...
        return probs   # shape [B, n_classes]
    ...
    def hybrid_score(self, x, centroids, w_ae=0.5, w_cls=0.5):
        """Hybrid = w_ae * AE_RE  +  w_cls * (1 - max_prob)"""
...
class FocalLoss(nn.Module):
    def __init__(self, n_classes, gamma=2.5, label_smooth=0.05, class_weights=None):
        ...
        self.nc     = n_classes
        self.w      = class_weights  # tensor [n_classes]
...
class SupConLoss(nn.Module):
    def __init__(self, T=0.15):  # T↑ 0.07→0.15: tránh exp overflow → NaN
        super().__init__()
        self.T = T

    def forward(self, feats, labels):
        B    = feats.shape[0]
        # Clamp sim trước khi exp để tránh overflow
        sim  = torch.matmul(feats, feats.T) / self.T
        sim  = sim - sim.detach().max()   # stable log-sum-exp shift
        ...
        lp   = sim - torch.log(denom)   # log-softmax over negatives
...
class IDSLoss(nn.Module):
    ...
    def forward(self, logits, labels, embeds, x, x_hat):
        ...
        # Clamp từng thành phần để phòng NaN lan truyền
...
def train_epoch(model, loader, optimizer, criterion, device):
    ...
    for X, y in loader:
        ...
        if use_amp:
            ...
            # Bỏ qua step nếu gradient có NaN/Inf
        ...
        if not np.isfinite(loss_val):
            continue   # bỏ qua batch NaN, không cộng vào thống kê
...
def build_centroids(model, X_tr, y_tr, n_clusters=25, device='cpu'):
    ...
    # ── FIX: thay NaN/Inf bằng 0 trước khi đưa vào KMeans ──
    ...
    for cls in range(len(np.unique(y_tr))):
        ...
        if len(cv) == 0:
            continue
        # Loại bỏ hàng vẫn còn NaN sau nan_to_num (phòng thủ)
...
def evaluate_zero_day(model, X_kn, y_kn, X_zd, y_zd, thr, centroids, device):
    ...
    for m in ['gradbp_l2','hybrid','ae_re','energy','softmax','fv_cluster']:
        ...
        tag = ' ◀ paper' if m=='gradbp_l2' else (' ◀ hybrid' if m=='hybrid' else '')
        print(f'  {m:<16} {auc:>8.4f} {t1:>10.4f} {t5:>10.4f}{tag}')
        results[m] = {'auc':auc,'tpr_1':t1,'tpr_5':t5,
                      'fpr':fpr_a.tolist(),'tpr':tpr_a.tolist(),
                      's_known':sk[m],'s_zd':sz[m]}

    # Per-class recall (best method)
...
# ═══════════════════════════════════════════════════════════════
# PLOTS  (giống v5 style)
# ═══════════════════════════════════════════════════════════════
def _attack_probs_batch(model, X, device, batch=512):
    """Returns max attack probability (excluding Normal class=0)."""
    model.eval()
    prob_attack = []
    with torch.no_grad():
        for i in range(0,len(X),batch):
            ...
            # prob of being ANY attack = 1 - P(Normal)
...
def plot_soc_decision_space(model, X_kn, y_kn_labels, X_zd, p_thr, re_thr,
    ...
    """
    SOC Decision Space:
      x-axis : P(Attack) from classifier
      y-axis : Reconstruction Error from AE (log scale)
      colors : Benign(blue) | Known Attack(red/pink) | Zero-Day(orange)
    """
    model.eval()
    ...
    # Known: separate benign vs attack
    ...
    Xb, yb_  = sample(X_kn[mask_benign], y_kn_labels[mask_benign], n_sample)
    Xka, yka_ = sample(X_kn[mask_katk],  y_kn_labels[mask_katk],   n_sample)
    ...
    # Thresholds
    ax.axvline(p_thr,  color='white',  linestyle='--', lw=1.2,
               label=f'P-Thr {p_thr:.2f}')
    ax.axhline(re_thr, color='white',  linestyle=':',  lw=1.2,
               label=f'RE-Thr {re_thr:.3f}')
    ...
    legend = ax.legend(facecolor='#1a1f2e', edgecolor='#555', labelcolor='white',
...
def plot_per_class_detection(clf_results, zd_results, label_names, save_path, normal_idx=0):
    """
    Per-class detection performance bar chart (v5 style, horizontal, 2-panel).
    Left : Known attacks (Supervised Head)
    Right: Zero-day attacks (Hybrid Anomaly)
    """
    # --- Known: detection rate = recall from classification report
    preds  = clf_results['preds']
    y_true = None  # reconstructed below
    probs  = clf_results['probs']

    # Per-class recall for known
    known_classes = [n for i,n in enumerate(label_names)]
    known_recall  = {}
    # We compute detection rate = how many of each class are flagged as attack
    # (not classified as Normal)
    # Actually we measure: for attack classes, rate classified ≠ Normal
    # For Benign class: FPR (classified as attack)

    # --- Zero-day: per class recall from zd_results
    per_cls_zd = zd_results.get('_per_class', {})
    zd_classes  = sorted(per_cls_zd.keys())
    zd_recalls  = [per_cls_zd[c]['recall']*100 for c in zd_classes]

    fig, axes = plt.subplots(1, 2, figsize=(18, 7))
    fig.patch.set_facecolor('#0d1117')

    # We'll just use the classification report F1 as proxy for detection
    # Actually compute per-class "detection rate" from probs:
    # For each known class: % of samples with P(attack) > 0.5
    # (known result doesn't have y_test stored — pass label_names and preds)
    # We use preds directly: pred != normal_idx
    kn_det = {}
    for i, name in enumerate(label_names):
        # We don't have per-class y_test here; use the classification report
        pass

    # Simpler: use clf_results stored info
    # We stored preds and probs; reconstruct per-class from confusion
    # Since we pass y_test from outside in main, let's just use stored probs
    # For each class: detection = avg prob of NOT being normal
    # This is approximate but visually correct
    known_dr = {}
    for i, name in enumerate(label_names):
        known_dr[name] = float(np.mean(probs[:,i] * (1 - probs[:,normal_idx])))

    # Better: use stored per_class from zd
    # Actually we'll pass it properly from caller — see run_full()

    colors_k = ['#00BFFF' if n=='Normal' else '#00A8E8'
                for n in label_names]

    # ── Left panel: Known ──
    ax = axes[0]
    ax.set_facecolor('#0d1117')
    # Placeholder bars using probs-based estimate (overridden in run_full)
    names_k = list(known_dr.keys())
    vals_k  = [known_dr[n]*100 for n in names_k]
    bars = ax.barh(names_k, vals_k, color='#00A8E8', height=0.6)
    for bar, val in zip(bars, vals_k):
        ax.text(bar.get_width()+0.5, bar.get_y()+bar.get_height()/2,
                f'{val:.1f}%', va='center', color='white', fontsize=10)
    ax.set_xlim(0, 110)
    ax.set_xlabel('Detection Rate (%)', color='white')
    ax.set_title('Known Attacks Detection Rate (%)\n(Supervised Head)',
                 color='white', fontsize=11)
    ax.tick_params(colors='white')
    ax.set_facecolor('#0d1117')
    for sp in ax.spines.values(): sp.set_edgecolor('#333')

    # ── Right panel: Zero-Day ──
    ax2 = axes[1]
    ax2.set_facecolor('#0d1117')
    bars2 = ax2.barh(zd_classes, zd_recalls, color='#FFA500', height=0.6)
    for bar, val in zip(bars2, zd_recalls):
        ax2.text(bar.get_width()+0.5, bar.get_y()+bar.get_height()/2,
                 f'{val:.1f}%', va='center', color='white', fontsize=10)
    ax2.set_xlim(0, 110)
    ax2.set_xlabel('Detection Rate (%)', color='white')
    ax2.set_title('Zero-Day Attacks Detection Rate (%)\n(Hybrid Anomaly)',
                  color='white', fontsize=11)
    ax2.tick_params(colors='white')
    for sp in ax2.spines.values(): sp.set_edgecolor('#333')

    fig.suptitle('V14.0 - PER-CLASS DETECTION PERFORMANCE (Zero-Day vs Known)',
                 color='white', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight', facecolor='#0d1117')
    plt.close()
    print(f'  [Plot] Per-class detection → {save_path}')


def plot_per_class_proper(label_names, y_test, preds, per_cls_zd, zd_classes_order,
                          save_path, normal_idx=0):
    """
    Proper per-class bar chart using actual recall from classification.
    Left : Known (recall per class from y_test vs preds)
    Right: Zero-day (recall from zd_results)
    """
    # Known: actual recall
    ...
    for i, name in enumerate(label_names):
        ...
        if i == normal_idx:
            # FPR for normal: pct classified as attack
            fpr = (preds[mask] != normal_idx).mean() * 100
            known_names.append(name.upper())
            known_recalls.append(fpr)
        else:
    ...
    plt.savefig(save_path, dpi=150, bbox_inches='tight', facecolor='#0d1117')
    plt.close()
    print(f'  [Plot] Per-class proper → {save_path}')
...
def plot_training_curve(history, save_path):
    ...
    fig.patch.set_facecolor('#0d1117')
    titles  = ['Train Loss', 'Val AUC', 'Val Accuracy']
    data    = [losses, aucs, accs]
    colors  = ['#FF6B6B','#00BFFF','#FFA500']
...
def plot_roc_curves(zd_results, save_path):
    """ROC curves for all detection methods."""
    ...
    ax.tick_params(colors='white')
    for sp in ax.spines.values(): sp.set_edgecolor('#333')
    legend = ax.legend(facecolor='#1a1f2e', edgecolor='#555', labelcolor='white', fontsize=9)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight', facecolor='#0d1117')
    plt.close()
...
def save_artifacts(model, splits, thresholds, history, centroids, save_dir):
    ...
    # .pkl — scaler + label_encoder + feat_cols + centroids
...
def run_full(args):
    ...
    print(f'  Params: {n_params:,}  |  n_feat={splits["n_features"]}  n_cls={splits["n_classes"]}')
    ...
    # Compute AE threshold from val set
    model.eval()
    re_val = []
    with torch.no_grad():
    ...
    p_thr  = 0.5  # decision boundary for attack probability
    ...
    clf_res     = evaluate_classifier(model, splits['X_test'], splits['y_test'],
                                       label_names, device)
    clf_res['y_test'] = splits['y_test']  # store for plots
    ...
    plot_training_curve(history,
                        os.path.join(args.plot_dir,'v14_training_curve.png'))

    # SOC Decision Space
    plot_soc_decision_space(
        model, splits['X_test'], splits['y_test'],
        splits['X_zd'], p_thr, re_thr, device,
        os.path.join(args.plot_dir,'v14_decision_space.png'),
        label_names,
    )

    # Per-class detection (proper)
    per_cls_zd = zd_res.get('_per_class',{})
    zd_cls_order = sorted(per_cls_zd.keys())
    plot_per_class_proper(
        label_names, splits['y_test'], clf_res['preds'],
        per_cls_zd, zd_cls_order,
        os.path.join(args.plot_dir,'v14_per_class_detection.png'),
        normal_idx=normal_idx,
    )

    # ROC curves
    plot_roc_curves(zd_res,
                    os.path.join(args.plot_dir,'v14_roc_curves.png'))
    ...
    best_tpr    = max((v.get('tpr_1',0) for k,v in zd_res.items()
                       if not k.startswith('_')), default=0.)

    print(f'\n{"="*70}')
    print('FINAL SUMMARY — IDS v14.0 (UNSW-NB15)')
    print(f'{"="*70}')
    print(f'  Parameters     : {n_params:,}')
    print(f'  n_features     : {splits["n_features"]}')
    print(f'  n_classes      : {splits["n_classes"]}  {splits["known_cats"]}')
    print(f'  Zero-day cats  : {splits["zd_cats"]}')
    print(f'  Known AUC      : {clf_res["auc"]:.4f}')
    print(f'  Best ZD AUC    : {best_zd_auc:.4f}')
    print(f'  Best TPR@1%FPR : {best_tpr:.4f}')
    print(f'  Model .pth     : {pth_p}')
    print(f'  Pipeline .pkl  : {pkl_p}')
    print(f'  Plots dir      : {args.plot_dir}')
    print(f'{"="*70}')

    # JSON results
    json_path = os.path.join(args.save_dir,'ids_v14_results.json')
    summary = {
        'model':       'IDS-v14.0-Hybrid-UNSW-NB15',
        'n_params':    n_params,
        'known_auc':   clf_res['auc'],
        'zd_results': {k:{ky:v for ky,v in r.items()
                          if ky not in ('fpr','tpr','s_known','s_zd')}
                       for k,r in zd_res.items() if not k.startswith('_')},
        'thresholds':  thresholds,
        'known_cats':  splits['known_cats'],
        'zd_cats':     splits['zd_cats'],
    }
...
def run_demo(args):
    ...
    # Simulate DoS hard (closer to Normal)
    dos_idx_demo = 1
    ...
    le = LabelEncoder()
    le.fit([f'Class_{i}' for i in range(n_cls)])
    splits = dict(
        ...
        label_encoder=le,scaler=RobustScaler().fit(X_tr),
    ...
    args.epochs   = min(getattr(args,'epochs',30),30)
    args.patience = min(getattr(args,'patience',8),8)

    criterion = IDSLoss(n_classes=n_cls, lambda_con=0.3, focal_gamma=2.0,
                        dos_class_idx=dos_idx_demo, dos_weight=5.0, device=device)
    loaders = make_loaders(splits, batch_size=256, dos_class_idx=dos_idx_demo)
    model, history = train(model, loaders, args, criterion, device)
    ...
    per_cls_zd = zd_res.get('_per_class',{})
    zd_cls_order = sorted(per_cls_zd.keys())
    plot_per_class_proper(label_names, y_te, clf_res['preds'], per_cls_zd,
                           zd_cls_order,
                           os.path.join(args.plot_dir,'v14_per_class_detection.png'),
                           normal_idx=0)
